"""BNB Chain live provider — Etherscan V2 hosted explorer (Phase 20, WP-06)."""
...
class BnbProvider(BlockchainProvider):
    ...
    def __init__(
        ...
        # No hardcoded URL — must come from env via Settings.bnb_provider_url
        # (see .env.example: BNB_PROVIDER_URL). Tests inject via Settings.
        base = (self._settings.bnb_provider_url or "").strip()
    ...
    def _ensure_configured(self) -> None:
        if not self._base_url:
            raise ProviderError(
                "bnb_provider_url not configured — set BNB_PROVIDER_URL / "
                "bnb_provider_url in env/.env (e.g. https://api.etherscan.io/v2/api)"
            )
    ...
    async def get_balance(self, address: str) -> Decimal:
        ...
        try:
            return Decimal(str(data)) / _WEI_PER_BNB
        except (InvalidOperation, ValueError, TypeError) as exc:
            raise ProviderError(f"BNB Chain provider: bad balance payload: {data!r}") from exc
    ...
    async def get_block_height(self) -> int:
        ...
        try:
            ...
            raise ProviderError(f"BNB Chain provider unreachable: {exc}") from exc
        result = payload.get("result")
        if isinstance(result, str) and result.startswith("0x"):
            try:
                return int(result, 16)
            except ValueError as exc:
                raise ProviderError(f"BNB Chain provider: bad block height: {result!r}") from exc
        raise ProviderError(f"BNB Chain provider error: {payload.get('message', payload)!r}")
    ...
    async def _account_list(self, action: str, address: str) -> list[dict[str, Any]]:
        ...
        for attempt in range(2):
            ...
            if self._is_rate_limit(message, detail) and attempt == 0:
                ...
                continue
            raise ProviderError(f"BNB Chain provider error: {message!r} ({detail})")
        raise ProviderError(f"BNB Chain provider error: {payload.get('message', payload)!r}")
    ...
    def _fee(row: dict[str, Any]) -> Decimal:
        try:
            ...
            return (used * price) / _WEI_PER_BNB
        except (InvalidOperation, ValueError):
            return Decimal("0")
