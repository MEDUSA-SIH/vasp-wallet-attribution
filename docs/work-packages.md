...
| WP-11 | Attribution Stage B — graph traversal (BFS + Dijkstra)                                              |             | `feature/attr-stage-b`              | 10               | ⬜      |
| WP-12 | Attribution Stage C — filtering (VASP heuristics, dust, mixers)                                  |             | `feature/attr-stage-c`              | 10               | ⬜      |
| WP-13 | Attribution Stage D — evidence collection                                                          |             | `feature/attr-stage-d`              | 10               | ⬜      |
| WP-14 | Attribution Stage E + F — proximity + confidence scoring                                           |             | `feature/attr-stage-ef`             | 10               | ⬜      |
| WP-15 | Attribution Stage G — ranking                                                                       |             | `feature/attr-stage-g`              | 10               | ⬜      |
| WP-16 | Attribution Stage H — explainability                                                                 |             | `feature/attr-stage-h`              | 10               | ⬜      |
| WP-17 | Graph store — Neo4j backend (replace NetworkX)                                                     |             | `feature/graph-neo4j`               | 6, 11            | ⬜      |
| WP-18 | Graph algorithms — community detection, page-rank, time-window traversal                           |             | `feature/graph-algos`               | 6, 11            | ⬜      |
| WP-19 | Risk typologies — catalog expansion (mixer, peel-chain, nested VASP, bridge abuse)                 |             | `feature/risk-typologies`           | 14               | ⬜      |
| WP-20 | Risk alerts — rule engine implementation                                                            |             | `feature/risk-alerts`               | 14               | ⬜      |
| WP-21 | Cross-chain bridge catalogue expansion + detection (Phase 13)                                       |             | `feature/cross-chain-bridges`       | 13               | ⬜      |
| WP-22 | SAHYOG adapter — real HTTP implementation                                                          |             | `feature/sahyog-real-gateway`       | 7                | ⬜      |
| WP-23 | SAHYOG inbound queue (background worker)                                                            |             | `feature/sahyog-inbound`            | 7, 23            | ⬜      |
| WP-24 | Reports — PDF / DOCX rendering (Phase 17)                                                          |             | `feature/reports-render`            | 17               | ⬜      |
| WP-25 | Evidence packaging service — Phase 16                                                               |             | `feature/evidence-package`          | 16               | ⬜      |
| WP-26 | Case service — full CRUD (Phase 12)                                                                 |             | `feature/case-crud`                 | 12               | ⬜      |
| WP-27 | Investigator service — auth, RBAC, password reset                                                  |             | `feature/auth-rbac`                 | 25               | ⬜      |
| WP-28 | Audit pipeline — middleware + persistence (Phase 25)                                                |             | `feature/audit-pipeline`            | 25               | ⬜      |
| WP-29 | Background workers — Celery / RQ setup (Phase 23)                                                  |             | `feature/worker-runtime`            | 23               | ⬜      |
| WP-30 | Demo seed dataset (`data/synthetic/`) + reproducible CSV / parquet generator                       |             | `feature/demo-dataset`              | 21, 22           | ⬜      |
| WP-31 | Observability — OpenTelemetry traces + metrics (Phase 25)                                            |             | `feature/otel`                      | 25               | ⬜      |
| WP-32 | Production deployment manifests (Helm / k8s)                                                       |             | `feature/prod-deploy`               | 24               | ⬜      |
| WP-33 | Frontend skeleton (separate repo / `packages/web/`)                                                |             | `feature/web-scaffold`              | —                | ⬜      |
